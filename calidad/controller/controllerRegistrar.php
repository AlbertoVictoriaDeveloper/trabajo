...
class controllerRegistrar {
   ...
   public function  updateStatuAudio($total,$idAudio,$comentarioObservaciones,$idUser,$nombreUser){
         ...
         $queryUpdate = "UPDATE `audio` SET `status_audio`='0', `fecha_res`='".$fecha."', `calificacion`='".$total."',observaciones = '".$comentarioObservaciones."',id_moni = '".$idUser."',user_moni = '".$nombreUser."'  WHERE (`id`='".$idAudio."')"; 
         return  $this->reversasModel->getUsuario($queryUpdate); 
     ...
     public function  saveAudio($nombreCampana,$fechaAudio,$agente,$clave,$tel,$status,$ruta,$duration2,$segundos){

     $query = "INSERT INTO `calidad`.`audio`
(
`nombre_campana`,
`fecha_gestion`,
`agente`,
...
'".$status."',
 ...
 public function guardarVariables($respuestas,$statusRespuesta,$idUsuario,$variables,$idAudio ){
    $fecha = date("Y-m-d H:i:s"); 
     $query = "INSERT INTO `calidad`.`detallescalifaciones`
(`respuesta`,
`status_respuesta`,
`fecha_respuesta`,
`catVariables_id`,
...
'".$idAudio."');"; 
 ...
 public function guardarAsignacion($opcion,$user){
 $query = "INSERT INTO `asignacion_personal` (
	`id_audio`,
	`estatus_regla`,
	`user`
)
VALUES
 ...
 public function getAgentesValidar($idUser,$motores){
     $query = "select id_agente from  asignacion_random  where asignacion_random.id_monitorista = '".$idUser."'and  motor_marcacion = '".$motores."' and 
asignacion_random.estatus_regla = 1 " ; 
 ...
 public function validacion($variables){
   $query = "select * from catvariables where tipo_variable = '".$variables."' ORDER BY id asc";
           return $this->reversasModel->getUsuario($query);
 ...
 public function getMonitorista(){
     $query = "select monitorista.nombre_validador,monitorista.num_empleado from monitorista where monitorista.monitoreo = 1";
      return $this->reversasModel->getUsuario($query); 
 ...
 public function  getAudiosAgent($numAgente){
     $query = "select * from audio WHERE audio.agente = '".$numAgente."' and status_audio = 1 ORDER BY RAND() LIMIT 1"; 
     return $this->reversasModel->getUsuario($query); 
     
 }

 
 public function  getAudioId($idAudio){
     $query = "select * from audio where audio.id = '".$idAudio."' LIMIT 1"; 
     return $this->reversasModel->getUsuario($query); 
 ...
 public function  getNombreAgente($idUser){
     $query = "select id,num_empleado from monitorista where monitorista.num_empleado = '".$idUser."'"; 
     return $this->reversasModel->getUsuario($query); 
